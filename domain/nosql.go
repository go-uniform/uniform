...
type INoSql interface {
	...
	Inc(timeout time.Duration, database, collection string, amounts map[string]float64, query bson.D) ([]interface{}, int64)
	...
	UpdateMany(timeout time.Duration, database, collection string, document interface{}, query bson.D) ([]interface{}, int64)
